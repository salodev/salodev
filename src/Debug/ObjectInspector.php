...
use salodev\Debug\ObjectInspector\URLHandler;

/**
 * It is a tool for browse complex objects structure.
 ...
 * a simple like xPath string. With this path we tell to ObjectInspector what
	...
	static private function _dump($element, $prevPath = null, $matchPath = null, $level = 0, $showElementType = true) {
		...
		if (is_array($element)) {
			...
			if (is_object($element)) {
				...
				$indexedConstants = array();
				foreach($constants as $name => $value) {
					$indexedConstants[$name] = "{$br}{$tabs}{$tab}{$pAccess} " . self::colorContent($name, 'limegreen') . ' ' . self::getPrintableValueType($value);
				}
				ksort($indexedConstants); echo implode('',array_values($indexedConstants));
				...
				foreach($properties as $property) {
					...
					$indexedProperties[($property->isStatic()?'1':'2') . $visibilityOrder[$access] . $property->name] = $property;
				}
				ksort($indexedProperties);
				$properties = array_values($indexedProperties);
				foreach($properties as $property) {
					$access = $property->isPrivate() ? 'private' : ($property->isProtected() ? 'protected' : 'public');
				...
				if (true) {
					...
					foreach($methods as $method) {
						...
						$pAccess = self::colorContent($access, 'dimgray', 'italic');
						$pFinal = self::colorContent($final, 'dimgray', 'italic');
						$pStatic = self::colorContent($static, 'dimgray', 'italic');
						$html .= "{$br}{$tabs}{$tab}{$pStatic}{$pAccess}{$pFinal} " . self::colorContent(($method->isStatic()?'::':'') . "{$method->name}(", 'blue',null, $method->getDocComment());
						$tmpParameters = array();
						foreach($method->getParameters() as $parameter) {
							$tmpParameters[] = self::colorContent("\${$parameter->name}", 'limegreen');
						}
						$html .= implode(', ', $tmpParameters);
						$html .= self::colorContent(")", 'blue');

						$methodIndex = ($method->isStatic()?'1':'2') . $visibilityOrder[$access] . $method->name;
						// echo "$access {$method->name} $methodIndex<br />";
						$indexedMethods[$methodIndex] = $html;
					}
					ksort($indexedMethods);
					echo implode('', array_values($indexedMethods));
				} else {
					echo "{$br}{$tabs}{$tab}methods... [+]";
				}
				if ($showElementType) {
					echo "{$br}{$tabs}".self::colorContent('}', 'blue');
				}
